	...
	for peerID, peer in pairs(self.GameClient.Peers) do
		if peer.Players then
			for playerID, player in pairs(peer.Players) do
				local x, y = Camera:worldToScreen(player.unit.b2Body:getPosition())
				local color = self.colors[peerID]
				if not color then color = self.colors[0] end
				love.graphics.printf(peer.name, x - 100, y - 35 * game.Camera.scale, 200, "center")
				love.graphics.setColor(color.r, color.g, color.b)
				love.graphics.printf(player.unit.health, x - 50, y - 28 * game.Camera.scale, 100, "center")
